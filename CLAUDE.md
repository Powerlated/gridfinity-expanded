...
- `cargo run -- optimize <in.toml> --mode <bins|walls> -o <out> [--format <stl|parasolid_x_t>] [--view]` — headless drawer
  ...
  object, and the two produce entirely different sets of parts out of one file, so an invocation that
  has not said which it wants has not said what it wants built. **An instance the packer cannot place
...
`--mode walls` fits.** A discrete bin is a whole number of cells on each axis, and one object's
