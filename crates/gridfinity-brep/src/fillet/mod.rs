...
use crate::topo::{Builder, EdgeFaces, EdgeId, Solid};
...
fn fillet_edges_with(
    ...
    let _perf = crate::perf::scope(crate::perf::Metric::FilletEdges);
    ...
    for fid in 0..s.faces.len() {
        if let Some(x) = crate::audit::face_loop_self_crossing(&s, fid) {
            return Err(format!(
