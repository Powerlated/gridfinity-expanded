...
mod tests {
    ...
    use super::super::fixtures::{blended_solid, cube_solid, lofted_solid, split_blended_solids};
    use super::*;
    ...
    fn battery() -> Vec<(&'static str, String)> {
        ...
        vec![
            ...
            ("a loft", to_xt_text(&[&lofted]).expect("a loft exports")),
            (
                "a cut blended prism, two bodies",
                to_xt_text(&split_refs).expect("the cut prism exports"),
            ),
            (
                "a cube and a loft, list-rooted",
    ...
    fn a_chart_point_off_its_surface_is_reported() {
        ...
        let text_out = to_xt_text(&refs).expect("the cut prism exports");
        let mut p = reader::parse(&text_out).expect("a clean file parses");
        let isect = p
            .nodes
            .iter()
            .find(|n| n.ty == text::INTERSECTION)
            .expect("the cut prism holds an intersection")
            .index;
