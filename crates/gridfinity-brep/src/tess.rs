use crate::math::{Vec2, Vec3, weld_key};
...
pub fn tessellate(solid: &Solid, arc_segs_per_quarter: usize) -> Tessellation {
    ...
    let _perf = crate::perf::scope(crate::perf::Metric::Tessellate);
    let out = tess_faces(solid, arc_segs_per_quarter);
    let leaks = crate::audit::tessellation_leaks(&out);
    assert!(
        leaks.is_empty(),
...
fn tess_faces(solid: &Solid, arc_segs_per_quarter: usize) -> Tessellation {
    ...
    for (fi, face) in solid.faces.iter().enumerate() {
        ...
        let flat_normal = match face.surface {
            crate::geom::Surface::Plane { normal, .. } => Some(*normal * sign),
            _ => None,
        };
        ...
        if matches!(face.surface, crate::geom::Surface::Torus { .. }) {
            for &(s, e) in &sc.spans {
...
fn triangulate_into(sc: &mut Scratch) {
    ...
    let (s, e) = sc.spans[0];
    if !crate::planar::span_ccw(&sc.uv, s, e) {
