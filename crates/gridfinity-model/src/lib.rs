//! The parametric Gridfinity model, stated over `gridfinity-brep`.
//!
//! `gridfinity` is the model proper -- a `Params` in, a watertight `Solid` or a
//! list of printable `BinPiece`s out -- and `layout` is the vocabulary it is
//! stated in: cells, the edges between them, and how a cell set partitions into
//! pieces. `printers` answers whether a piece fits a bed and where to cut it if
//! it does not, and `subbin` builds the separately printed insert that stands
//! in a compartment. Nothing here is a kernel operator: every solid is assembled
//! from `gridfinity_brep`'s sketches, rings and blends, and `perf` claims the
//! kernel profile's user slots for the scopes a bin's plan passes through.
...
pub use gridfinity_brep::mesh::Mesh;
pub use gridfinity_brep::tess::{Tessellation, tessellate, tessellate_shell};
pub use gridfinity_brep::topo::Solid;
pub use gridfinity_brep::xt::to_xt_text;

#[cfg(test)]
#[global_allocator]
static TEST_ALLOC: gridfinity_brep::perf::CountingAlloc<mimalloc::MiMalloc> =
...
mod tests {
    ...
    /// The cells a `w` x `d` mm drawer holds at `pitch`: the whole cells that
    /// fit on each axis, row-major, with the leftover millimetres left over.
    /// This is what `gridfinity-project`'s `drawer_grid` floors to, restated
    /// here because the model must not depend on the fitter that consumes it.
    fn drawer_cells(w: f64, d: f64, pitch: f64) -> Vec<GridCell> {
        let (nx, ny) = ((w / pitch).floor() as i32, (d / pitch).floor() as i32);
        assert!(
            nx > 0 && ny > 0,
            "a {w} x {d} mm drawer at a {pitch} mm pitch holds no whole cell"
        );
        (0..ny)
            .flat_map(|y| (0..nx).map(move |x| GridCell { x, y }))
            .collect()
    }
    ...
    fn a_drawer_sized_baseplate_carves_into_printable_pieces() {
        ...
        let cells = drawer_cells(400.0, 300.0, gridfinity::GRID_PITCH);
        ...
        solid
            ...
            .filter(|f| match f.surface {
                geom::Surface::Torus { center, axis, .. } => {
                    ...
                    let p = gridfinity_brep::math::Vec2::new(center.x, center.y);
                    let l2 = ab.dot(ab);
                    let t = if l2 < 1e-9 {
                        0.0
                    } else {
...
{}",
    ...
    fn perf_counters_see_a_real_build() {
        use gridfinity_brep::perf::{self, Metric};
        let _g = perf_guard();
        ...
        for want in ["gridfinity::plan_piece", "  plan: cavity", "  plan: rings+stitch"] {
            assert!(
                rows.iter().any(|r| r.name == want && r.calls > 0),
                "the model's own scope {want} must reach the same table the kernel's do,                  which is the whole point of it claiming a user slot rather than keeping                  counters of its own"
            );
        }
        assert!(
            rows.iter().all(|r| !r.name.is_empty()),
            "an unclaimed user slot is no row at all, so nothing nameless may reach a report"
        );
    }

    #[test]
    #[ignore = "benchmark: cargo test --release -p gridfinity-model --lib -- --ignored --nocapture"]
    fn alloc_report() {
...
mod audit_tests {
    ...
    use gridfinity_brep::topo::{Builder, Loop};
