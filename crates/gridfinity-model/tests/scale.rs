...
use gridfinity_model::gridfinity::{LogicalBin, Params, try_build};
use gridfinity_brep::tess::tessellate;
use gridfinity_model::layout::GridCell;
...
fn pipeline_bench() {
    ...
    for _ in 0..3 {
        let t = Instant::now();
        let (tx, rx) = sync_channel::<gridfinity_model::Solid>(1);
        let fs = frames.clone();
...
fn tess_bench() {
    ...
    let mut tris = 0;
    let _ = gridfinity_brep::tess::tess_diag();
    for _ in 0..25 {
        let t = Instant::now();
        let tess = tessellate(&solid, 4);
    ...
    let d = gridfinity_brep::tess::tess_diag();
...
fn plan_bench() {
    ...
    for _ in 0..15 {
        let t = Instant::now();
        let prog = gridfinity_model::gridfinity::program(&p);
        best = best.min(t.elapsed().as_secs_f64() * 1e3);
        ops = prog.steps.len();
    }
