...
use crate::surf::{self, GeomLinks, ON_GEOMETRY_MM};
...
pub fn plan(
    ...
    let steps = match curve {
        Curve::Section { chart } => chart.len() - 1,
        _ => {
            let sweep = (t1 - t0).abs();
            let turns = (sweep / std::f64::consts::FRAC_PI_2) * CHART_PER_QUARTER;
            (turns.ceil() as usize).max(CHART_MIN_POINTS - 1)
        }
    };
