...
use crate::text::{self, Index, Writer};
...
/// The XT node for `curve` where the format has one -- `Line`, `Circle` and
/// `Ellipse` -- or `None` for the two it does not: a torus section and a
/// sampled section, which `isect` writes as the exact intersection of the two
/// surfaces they lie on instead.
