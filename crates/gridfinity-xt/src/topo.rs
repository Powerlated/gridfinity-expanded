...
use crate::math::{Dir, Vec3, WELD_NEAR_SQ, weld_key};
...
impl Edge {
    ...
    pub fn seg_count(&self, arc_segs_per_quarter: usize) -> usize {
        match self.curve {
            Curve::Line { .. } => 1,
            Curve::Section { ref chart } => chart.len() - 1,
...
impl Builder {
    ...
    /// The edge from `a` to `b` along the section `chart` describes, whose
    /// parameter runs 0 to 1 with the chart's own order. The chart must start
    /// at `a`'s point and end at `b`'s, which is what makes the edge's ends and
    /// its curve's ends the same places.
    pub fn section(&mut self, a: VertexId, b: VertexId, chart: Vec<Vec3>) -> (EdgeId, bool) {
        assert!(
            chart.len() >= 2,
            "a section edge carries a chart of at least two points, got {}",
            chart.len()
        );
        let curve = Curve::Section { chart };
    ...
    pub fn build_unvalidated(self) -> Solid {
        ...
        crate::orient::normalize(&mut solid);
        solid
    }
