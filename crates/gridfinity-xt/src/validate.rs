...
mod tests {
    use crate::text;
    use crate::to_xt_text;
    use crate::transmit::fixtures::{blended_solid, cube_solid, hollow_solid};
    use super::*;
    use crate::topo::Solid;

    /// The export battery: every shape class an OCCT body can put in a file,
    /// single- and multi-body, including the cut pieces whose corner-blend
    /// sections are the only input that reaches INTERSECTION.
    fn battery() -> Vec<(&'static str, String)> {
        let cube = cube_solid();
        let blended = blended_solid();
        let lofted = hollow_solid();
        let cut = crate::transmit::fixtures::cut_solids();
        let cut_refs: Vec<&Solid> = cut.iter().collect();
        vec![
            ("a cube", to_xt_text(&[&cube]).expect("a cube exports")),
            (
                "a blended prism",
                to_xt_text(&[&blended]).expect("a blended prism exports"),
            ),
            ("a hollow prism", to_xt_text(&[&lofted]).expect("a hollow prism exports")),
            (
                "a cube and a hollow prism, list-rooted",
                to_xt_text(&[&cube, &lofted]).expect("the pair exports"),
            ),
            (
                "a cut blended prism, two bodies",
                to_xt_text(&cut_refs).expect("the cut prism exports"),
            ),
        ]
    }
    ...
    fn every_export_in_the_battery_validates_clean() {
        ...
        let pair = battery()[3].1.clone();
        let parsed = reader::parse(&pair).expect("a clean file parses");
        assert!(
            parsed.nodes.iter().filter(|n| n.ty == text::BODY).count() == 2,
            "the battery holds a multi-body file, so the list-root checks are not vacuous"
        );
        let blended = reader::parse(&battery()[1].1).expect("a clean file parses");
        assert!(
            blended.nodes.iter().any(|n| n.ty == text::TORUS),
            "the battery holds a torus, so the torus form checks are not vacuous"
        );
        let cut = reader::parse(&battery()[4].1).expect("a clean file parses");
        assert!(
            cut.nodes.iter().any(|n| n.ty == text::INTERSECTION),
    ...
    /// The blended prism's text, parsed, for the corruption tests to work on.
    fn parsed_bin() -> reader::Parsed {
        let blended = blended_solid();
        reader::parse(&to_xt_text(&[&blended]).expect("a blended prism exports"))
    ...
    fn a_header_schema_mismatch_is_reported() {
        let blended = blended_solid();
        let text_out = to_xt_text(&[&blended]).expect("a blended prism exports");
