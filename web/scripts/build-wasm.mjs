/**
 * Compiles the Rust geometry kernel to WebAssembly into `web/src/wasm/`.
 *
 * The cargo workspace is rooted at this repository's root -- the parent of the
 * `web/` app this script lives in -- with the crates under `crates/`; set
 * `GRIDFINITY_KERNEL` to point somewhere else, relative to that root, to build
 ...
 *
 * With `--if-needed` it rebuilds only when `web/src/wasm/` is missing an artifact or
...
const outDir = resolve(webRoot, 'src/wasm');

if (!existsSync(cratePath)) {
  console.error(
    `Rust geometry kernel not found at ${cratePath}.\n` +
    'The kernel workspace should be the repository root, the parent of web/; set GRIDFINITY_KERNEL to use another checkout.',
  );
  process.exit(1);
}
...
function staleness() {
  ...
  if (missing.length > 0) return `web/src/wasm/ is missing ${missing.join(', ')}`;
  ...
  return changed > built ? 'the Rust kernel changed since web/src/wasm/ was built' : null;
}

if (process.argv.includes('--if-needed')) {
  const reason = staleness();
  if (reason === null) {
    console.log('web/src/wasm/ is up to date with the Rust kernel.');
    process.exit(0);
  }
  console.log(`Rebuilding web/src/wasm/: ${reason}.`);
}
