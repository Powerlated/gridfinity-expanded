...
**Don't run the long targets by default.** `--test fuzz` is for a deliberate pre-PR run or CI; say in the report when it was skipped. Run one profile by name when the change is in what it covers -- `tests/fuzz.rs` is a single generate/check/shrink path and each `#[test]` is an `Options` value aimed at one corner of the model, so the name tells you the coverage: `fuzz_inner_walls`, `fuzz_tidy_inner_walls`, `fuzz_wall_openings`, `fuzz_openings_and_inner_walls`, `fuzz_bin_shapes`, `fuzz_split_pieces`, `fuzz_params_broad`. A profile that finds a pre-existing undiagnosed defect names it in `Options::known` and keeps gating everything else; never add a signature there to silence something new. The benchmarks (`--test scale`, the `gridfinity-gui` badapple timings, `perf_report`, `alloc_report`) are `#[ignore]`d and only run when you ask for `--ignored`.
