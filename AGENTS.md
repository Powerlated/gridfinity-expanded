...
`crates/gridfinity-render` is a shared `wgpu` + `glam` crate with no egui or wasm deps, consumed by the egui debugger (`gridfinity-gui`) and the web app (`gridfinity-wasm`). Keep front-end concerns out; change its camera, shaders, or vertex format only with both consumers in mind.
...
- `cargo test --release --workspace -- --ignored --nocapture` — the benchmarks and perf reports, which are `#[ignore]`d so no ordinary run pays for them
