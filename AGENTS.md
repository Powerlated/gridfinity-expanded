...
- Preview data may be grouped, coloured and positioned; export data must preserve coordinates, topology, orientation, and per-piece meaning. Both consume the identical `BinPiece.vertices` buffer (`Tessellation::welded_render_buffer()`); preview offsets come from `previewLayout()` after generation, never from worker input or output. Never export the raw unwelded `render_buffer()` — it leaks.
...
**Don't run the long targets by default.** `--test fuzz`, `--test scale` and the `gridfinity-gui` benches are for a deliberate pre-PR run or CI; say in the report when they were skipped. Run one by name when the change is in what it covers.
