...
use crate::kernel::perf::{Metric, scope};
...
use crate::kernel::sketch::{COINCIDENT, Seg, Sketch, loop_area, point_in_segs, reverse_loop};
...
fn plan_cavities(
    ...
    let _g = scope(Metric::PlanCavity);
...
fn emit_base_and_rim(
    ...
    let _g = scope(Metric::PlanStitch);
    let (floor_z, total_h) = (outline.floor_z, outline.total_h);
